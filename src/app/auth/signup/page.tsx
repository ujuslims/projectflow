...
// Simple SVG for Google icon (copied from login page)
const GoogleIcon = () => (
...
export default function SignUpPage() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      await signInWithGoogle(); // Firebase handles new user creation
      toast({ title: "Google Sign-Up Successful", description: "Welcome! Redirecting..." });
      router.push('/projects');
    } catch (error: any) {
      ...
      if (error.code === 'auth/popup-closed-by-user') {
        ...
        errorMessage = "Network error during Google Sign-Up. Please check your connection.";
      }
      setFormError(errorMessage);
      toast({ title: "Google Sign-Up Error", description: errorMessage, variant: "destructive" });
    } finally {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <CardContent className="space-y-4">
          ...
          <Button type="submit" className="w-full" disabled={isLoading}>
            {(isSubmitting || (authLoading && !isGoogleLoading))&& <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            Sign Up
          </Button>
        </CardContent>
      </form>
